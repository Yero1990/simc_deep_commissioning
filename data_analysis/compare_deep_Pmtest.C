...
void compare_deep_Pmtest(int set, int pm, string model, string rad)
{
...
//Set Legend
   ...
   data_emiss->Draw("hist");
   simc_emiss->Draw("sameshist");
   leg25->AddEntry(data_emiss,"Data","f");
   leg25->AddEntry(simc_emiss,"SIMC");
   leg25->Draw();

   c3->SaveAs(Form("Kinematics_pm%d_set%d.pdf", pm, set));                                                                   
   ...
   leg_Pmx->AddEntry(simc_Pmy,"SIMC");
   leg_Pmx->Draw();
   ...
   leg_Pmy->AddEntry(simc_Pmx,"SIMC");
   leg_Pmy->Draw();
