...
for irun in (rfile):
    ...
    f = open('D2_REPORT/replay_coin_deep_check_%d_-1.report' % (irun))
    ...
    for line in f:
        ...
        if('Beam energy' in line):
            Eb.append(float(line.split(':')[1]))
        if('hHMS P Central' in line):
            hmsP.append(float(line.split(':')[1]))
        if('hHMS Angle' in line):
            hmsAng.append(float(line.split(':')[1]))
        if('pSHMS P Central' in line):
            shmsP.append(float(line.split(':')[1]))
        if('pSHMS Angle' in line):
            shmsAng.append(float(line.split(':')[1]))
        #Mispointings / BPMs
...
#Write to a table
fTable = open(runlist.split('.')[0]+'_report.dat', 'w')
fTable.write('#!Run[i,0]/   hmsP[f,1]/   hmsAng[f,2]/   shmsP[f,3]/   shmsAng[f,4]/   BCM4A_Current[f,1]/    BCM4A_Charge[f,2]/    h3of4_rate[f,3]/    p3of4_rate[f,4]/     coin_rate[f,5]/    htrk_eff[f,6]/   htrk_eff_err[f,7]/  ptrk_eff[f,8]/   ptrk_eff_err[f,9]/    cpuLT[f,10]/    tLT[f,11]/    x_bpm[f,12]/    y_bpm[f,13]/   hMisP_X[f,14]/    hMisP_Y[f,15]/    pMisP_X[f,16]/    pMisP_Y[f,17]/   ptrig6_accepted[i,18]/     ptrig6_scaler[i,19]/    edtm_accepted[i,20]/   edtm_scaler[i,21]/    pdid_e[i,22]/    pshould_e[i,23]/    hdid_h[i,24]/   hshould_h[i,25]/ \n')

output = '\n'.join('\t\t'.join(map(str,row)) for row in zip(run,hmsP,hmsAng,shmsP,shmsAng,pbcm4a_current,pbcm4a_charge,h3of4_rate,p3of4_rate,coin_rate,htrk_eff,htrk_eff_err,ptrk_eff,ptrk_eff_err,cpuLT,tLT,x_bpm,y_bpm,hMisP_X,hMisP_Y,pMisP_X,pMisP_Y,ptrig6_accepted,ptrig6_scaler,edtm_accepted,edtm_scaler,pdid_e,pshould_e,hdid_h,hshould_h))
fTable.write(output)
